 ...
 */
function useGlowEffect(containerRef: React.RefObject<HTMLElement>, cardSelector: string, throttleDelay: number = 16) {
  ...
  useEffect(() => {
    ...
    container.addEventListener('mousemove', throttledMouseMoveRef.current as EventListener);
    ...
    return () => {
      if (container) {
        if (throttledMouseMoveRef.current) {
          container.removeEventListener('mousemove', throttledMouseMoveRef.current as EventListener);
        }
